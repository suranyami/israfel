...
window.PlanarMap = class PlanarMap
  ...
  addVertex: (vertex) ->
    @vertices.push(vertex) unless vertex in @vertices
  ...
  removeElement: (array, item) ->
    array = array.splice(array.indexOf(item), 1) if item in array

  removeVertex: (vertex) ->
    for neighbour in vertex.neighbours()
      edge = neighbour.nextElement
      (edge.getOther vertex).removeEdge edge
      removeElement(@edges, edge)
